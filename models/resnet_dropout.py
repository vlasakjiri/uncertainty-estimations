...
class ResNetDropout(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, p=0.2):
        ...
        self.layer2 = self._make_layer(
            block, 128, num_blocks[1], stride=2, p=p)
        self.layer3 = self._make_layer(
            block, 256, num_blocks[2], stride=2, p=p)
        self.layer4 = self._make_layer(
            block, 512, num_blocks[3], stride=2, p=p)
...
def ResNet18Dropout(num_classes, p=0.2):
    return ResNetDropout(BasicBlock, [2, 2, 2, 2], num_classes=num_classes, p=p)


def ResNet34Dropout(num_classes, p=0.2):
    return ResNetDropout(BasicBlock, [3, 4, 6, 3], num_classes=num_classes, p=p)


def ResNet50Dropout(num_classes, p=0.2):
    return ResNetDropout(Bottleneck, [3, 4, 6, 3], num_classes=num_classes, p=p)


def ResNet101Dropout(num_classes, p=0.2):
    return ResNetDropout(Bottleneck, [3, 4, 23, 3], num_classes=num_classes, p=p)
